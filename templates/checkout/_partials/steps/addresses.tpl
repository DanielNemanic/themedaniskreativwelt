...
>
  ...
  {if !$use_same_address}
    <h4>{l s='Shipping Address' d='Shop.Theme.Checkout'}</h4>
  {/if}
  ...
  {elseif $customer.addresses|count > 0}
    ...
    {if isset($delivery_address_error)}
      <div class="uk-alert-danger js-address-error" uk-alert name="alert-delivery" id="id-failure-address-{$delivery_address_error.id_address}">{$delivery_address_error.exception}</div>
    {else}
      <div class="uk-alert-danger js-address-error" uk-alert name="alert-delivery" style="display: none">{l s="Your address is incomplete, please update it." d="Shop.Notifications.Error"}</div>
    {/if}

    <p class="add-address">
      <a href="{$new_address_delivery_url}"><span uk-icon="icon:plus"></span> {l s='add new address' d='Shop.Theme.Actions'}</a>
    </p>

    {if $use_same_address && !$cart.is_virtual}
      <p>
        <a class="uk-button uk-button-text" data-link-action="different-invoice-address" href="{$use_different_address_url}">
  ...
  {if !$use_same_address}
    ...
    <h4>{l s='Your Invoice Address' d='Shop.Theme.Checkout'}</h4>
  ...
  {if !$form_has_continue_button}
      <div class="uk-margin">
      <button type="submit" class="uk-button uk-width-1-1 uk-width-1-2@s uk-width-1-3@m uk-button-daniskreativwelt" name="confirm-addresses" value="1">
